# VERHallu
Official Implementation of ''VERHallu: Evaluating and Mitigating Event Relation Hallucination in Video Large Language Models''.




Our data is available at  https://www.alipan.com/s/vVmz3LQRmcw  !!!!


## Citation

If you find this repo helpful, please cite the following:

``` latex
@article{zhang2026verhallu,
  title={VERHallu: Evaluating and Mitigating Event Relation Hallucination in Video Large Language Models},
  author={Zhang, Zefan and Zhu, Kehua and Jiang, Shijie and Lu, Hongyuan and Sun, Shengkai and Bai, Tian},
  journal={arXiv preprint arXiv:2601.10010},
  year={2026}
}
```
